...
ray.init(
    ...
    log_to_driver=False,
    num_cpus=16,   # set to what the scheduler gave you
    num_gpus=1,
)
...
# alternative 
    ...
    # num_gpus=1,
    # num_cpus_per_worker= 1,
    # rollout_fragment_length=100, #steps each environment produces before sending data back - gives one planner decision per fragment
    # train_batch_size=3200,      # total number of transitions collected before one training update: 8 workers * 2 envs * 200 steps
    # sgd_minibatch_size=512, #how the training batch is split during optimization:
