...
# alternative 
    num_workers=num_workers, #4,  #8, #parallel rollout processes
    num_envs_per_worker=num_envs_per_worker, #2,  #environments internally (vectorized environments).
    num_gpus=1,
    num_cpus_per_worker= 1,
    ...
    train_batch_size=train_batch_size, #1600,  #3200,      # total number of transitions collected before one training update: 8 workers * 2 envs * 200 steps
    sgd_minibatch_size= sgd_minibatch_size,#400, #800 #how the training batch is split during optimization:
